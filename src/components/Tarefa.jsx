  ...
  return (
    <>
      {listaDeTarefas.map((tarefa) => (
        <div
          key={tarefa.id}
          className={tarefa.concluido ? "item concluido" : "item"}
        >
          <div className={mudarCorEtiqueta(tarefa.categoria)}>
            <p style={{ color: "white" }}>
              {mudarTextoEtiqueta(tarefa.categoria)}
            </p>
          </div>
          <div className="btnMarcarConcluida">
            <button onClick={() => marcarTarefaConcluida(tarefa.id)}></button>
          </div>
          <div className="textoTarefa">{verificarEdicao(tarefa)}</div>
          <div className="btnExcluirEditar">
            <button onClick={() => mudancaEdicao(tarefa.id)} className="editar">
              {mudarBotao(tarefa.id)}
            </button>
            <button
              onClick={() => removerTarefaDaLista(tarefa.id)}
              className="excluir"
            >
              🗑️
            </button>
